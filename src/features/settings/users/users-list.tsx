...
import { useDeleteUser } from './users.mutation'
import type { GetUsersResponse, UserStore } from './users.service'
...
export function UsersList({ response, isLoading, isError }: Props) {
  ...
  const [editingUser, setEditingUser] = useState<UserStore | null>(null)
  const [deletingUser, setDeletingUser] = useState<UserStore | null>(null)

  function handleDeleteConfirm() {
    if (!deletingUser) return

    deleteMutation.mutate(deletingUser.user.id)
